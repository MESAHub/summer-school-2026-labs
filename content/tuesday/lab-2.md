...
In the intermediate-age main sequence model (middle panel), $\nabla_{\mu} \neq 0$ due to hydrogen burning in the core. This produces a narrow spike in $N^2$ (see the new "bump" starting at a mass fraction of 0.2 and continuing until roughly 0.3). The spike partially reflects and traps some *g*-modes while others penetrate more deeply into the core. Because different modes now probe different regions, their periods are shifted by different amounts. This produces the characteristic "dips" or oscillatory deviations from a constant period spacing.
...
In the location you are working in, run:
```bash
...
> [!NOTE]
> For science cases, you may want to consider changing parameters in the `&eos` and `&kap` namelists. It isn't guaranteed that the defaults will always be right for your particular case.
...
**In the "starting specifications" section, set `initial_mass = 5d0`** to match the mass of our `zams.mod` model. We can leave the metallicity as it is.
> [!NOTE]
> It is not strictly necessary to set the mass in our inlist, since `zams.mod` should know. However, it is helpful to keep track of the mass this way. If the values in the inlist differ from the model values, `MESA` will overwrite with the model values.
...
```
Feel free to check your `&controls` namelist in the collapsible box below.
{{< details title="`&controls Namelist`" closed="true" >}}
```fortran
&controls
...
Increasing the resolution of a model can be a bit more computationally expensive depending on your machine. Lower values of `mesh_delta_coeff` will increase the runtime. It is then up to you to make an informed decision based on the specifications of your specific machine. The primary factor that will effect the runtime is the `OMP_NUM_THREADS` parameter you have set in your environment variables.

If you have `OMP_NUM_THREADS > 2`, **set mesh_delta_coeff = 0.5** in your inlist. If you have 2 or less, set it instead to 0.6.

## Task 5: Output, Output, Read All About It!
Next, we will configure the output settings so `MESA` writes files that can be used by `GYRE`. By default, `MESA` does not write pulsation-profile data. **Explore the "controls for output" section of the controls reference page and identify the flag that enables pulsation-profile output for `GYRE`. You will also need to tell `MESA` to write the files in FGONG format.**

Check the following hint if you get stuck or want to check your answer.
...
With the correct flag, `MESA` will write a unique `.data.FGONG` file for every profile it creates. Since we are only interested in specific stages, we will need to tell `MESA` when to write profiles. This will require some extra code in our `./src/run_star_extras.f90`, which we will get to later. 

In the meantime, **turn profile generation off by including the following line in the `&controls` section of your inlist**:
```fortran
profile_interval = -1
```
Since a negative interval isn't possible, `MESA` interprets this as disabling automatic profile output.

## Bonus Task: No History, No Problems
This step is optional, but it can help keep the working directory clean during repeated runs.

Since we are interested in only specific points in time for this lab and not the evolution of the star over it's whole lifetime, we don't need a full `history.data` file. Since we also do not plan to restart uninterrupted runs, photo files are unnecessary. **Turn history file and photo generation off with the following lines**:
```fortran
photo_interval = -1
...
If the model begins evolving and output appears in the terminal, the inlist is working correctly. **Once the run starts successfully, stop it with `Ctrl+C`**.
...
>[!NOTE]
> In this section, it is good practice to recompile any time you make any change to `run_star_extras.f90`. Even when you aren't explicitly told, feel free to run `./clean; ./mk` as often as you want!
...
- "`extras_start_step`", which runs once at the beginning of a model
- "`extras_finish_step`", which runs after each successfully converged timestep.

In `extras_start_step`, we will begin by storing the initial hydrogen abundance when the run starts. `MESA` provides the internal array `s% xtra(:)` for storing temporary data. We will store the initial central hydrogen abundance in `s% xtra(1)`. **Replace the entire `extras_start_step` subroutine with the following code**:
```fortran
      integer function extras_start_step(id)
         integer, intent(in) :: id
         integer :: ierr
...
**Go ahead and recompile to verify that everything is working**:
```bash
./clean; ./mk
```
...
**Add the following lines near the end of the `&controls` namelist in `inlist_project`**:
```fortran
   ! extras
   x_ctrl(1) = 0.6d0
   x_ctrl(2) = 0.3d0
...
{{< details title="Hint: Complete `run_star_extras.f90`" closed="true" >}}
If you got stuck on the previous step, copy these contents into your `./src/runs_star_extras.f90` file:
```fortran
! ***********************************************************************
!
...
```
{{< /details >}}
{{< details title="Hint: Complete `inlist_project`" closed="true" >}}
If you got stuck on the previous step, copy these contents into your `inlist_project` file:
```fortran
! inlist to evolve a 15 solar mass star
...
Begin with the `GYRE` inlist from Lab 1. If need a new copy, make a new file named `gyre.in` in your working directory and paste the contents of the following box:
{{< details title="Hint: `gyre.in` Template" closed="true" >}}
```fortran
&constants
/
...
```
If you did the bonus task in Lab 1, **remove the `detail_template` and `detail_item_list` lines as well**.
...
**Make a *personal copy* of [this](https://colab.research.google.com/drive/1HLO-UETt9uTcHpkvyozsZS4BrAUETmZS?usp=sharing) Google Colab notebook. Run the file ("Run All" in the control bar) and upload your summary files when prompted at the bottom of the cell.**
>[!NOTE]
> You must upload all three summary files at the same time. The code is written to run immediately after upload. If you need to restart, use the dropdown next to "Run All" and select "Restart session and run all."

The code should create a single plot showing the period-spacing patterns at the three evolutionary stages (hydrogen abundance of 0.6, 0.3, and TAMS).

The period-spacing patterns should show the characteristic dips associated with mode trapping caused by composition gradients. The dips should appear relatively regular in the model with a central hydrogen abundance of 0.5, but become less coherent by the time it reaches 0.3. As the star evolves, the internal composition structure becomes increasingly complex due to ongoing nuclear burning and mixing processes. By the TAMS, the period-spacing pattern should appear significantly noisier.
...
Take some time to talk with your table about these results.

Full solutions for this lab are available [here](https://drive.google.com/file/d/1xCz3KMPwY5_wLtPwiblynq50cfu5J1iL/view?usp=sharing)